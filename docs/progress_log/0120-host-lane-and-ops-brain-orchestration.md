...
3. `ops-brain` runbooks were refactored into bootstrap and services phases
4. Newt-on-Kubernetes scaffolding was added for `ops-brain`
...
1. `scripts/2-ops/ops-brain/01-bootstrap/`
2. `scripts/2-ops/ops-brain/02-services/`

Current behavior:
1. top-level `scripts/2-ops/ops-brain/00-run-all.sh` now really runs all known phases
...
2. `ops/helm/newt/values.ops-brain.yaml`
3. `ops/helm/newt/README.md`
4. Newt vars in `ops/ansible/group_vars/ops_brain.yml`
...
3. `ops-brain` and `vps` SSH ping scripts now reference host-side reseed scripts on failure
4. `ops-brain` `01-ansible-ping.sh` was corrected to be a pure reachability check (no `sudo` escalation)

## As Of This Update
These statements were true enough to rely on:
1. `ops-brain` bootstrap lane works through cluster verification
2. `ops-brain` has working Debian 12, SSH, sudo, k3s, Helm, label, and cluster verification
3. `pangolin-server` remains the VPS installer/runtime path, not a Kubernetes/Helm path
4. `newt` is now treated as a Kubernetes-native workload on `ops-brain`
...
2. store Newt credentials with `scripts/2-ops/host/10-write-ops-brain-pangolin-site-secret.sh`
