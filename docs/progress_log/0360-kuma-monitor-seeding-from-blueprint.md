...
2. The canonical endpoint source for Pangolin-exposed monitoring surfaces is:
   - `ops/pangolin/blueprints/observatory/monitoring.blueprint.yaml`
...
2. It reads Kuma admin credentials from 1Password item `kuma_observatory_admin`.
3. It opens a temporary SSH-backed tunnel to the in-cluster Kuma service on `observatory`.
