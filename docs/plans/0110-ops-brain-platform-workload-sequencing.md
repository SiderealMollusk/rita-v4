...
1. rebuild the 16 GB laptop as the `ops-brain`
...
Use bare-metal k3s on the laptop as the `ops-brain` control plane and initial monitoring node.

### Deliverables
1. New inventory entry and node doc for `ops-brain`
...
Detailed plan:
- `docs/plans/0120-ops-brain-k3s-monitoring-stack.md`
...
2. deployment boundaries between `platform-node`, `ops-brain`, `workload-node`, and VPS
...
1. add a node doc and inventory path for the laptop `ops-brain`
