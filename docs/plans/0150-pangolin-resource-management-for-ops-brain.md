...
## Goal
Move `ops-brain` service exposure from ad-hoc port-forwarding to repo-managed Pangolin resource declarations.

This plan starts after:
1. `pangolin-server` is working on the VPS
2. the `ops-brain` site exists in Pangolin
3. Newt is connected from `ops-brain`
4. the `ops-brain` monitoring stack is running
...
Examples:
1. `ops-brain-kube-prometheus-grafana.monitoring.svc.cluster.local:80`
2. `ops-brain-kube-prometheus-prometheus.monitoring.svc.cluster.local:9090`
3. `ops-brain-loki.monitoring.svc.cluster.local:3100`
...
1. blueprint artifacts:
   - `ops/pangolin/blueprints/ops-brain/`
2. host-side operator scripts:
   - `scripts/2-ops/host/`
3. service verification:
   - `scripts/2-ops/ops-brain/12-verify-monitoring-stack.sh`

## Proposed Blueprint Targets
### Grafana
1. site: Pangolin site identifier from `pangolin_site_ops_brain.identifier`
2. hostname:
   - `ops-brain-kube-prometheus-grafana.monitoring.svc.cluster.local`
3. method: `http`
4. port: `80`
...
1. site: Pangolin site identifier from `pangolin_site_ops_brain.identifier`
2. hostname:
   - `ops-brain-kube-prometheus-prometheus.monitoring.svc.cluster.local`
3. method: `http`
4. port: `9090`

## Execution Sequence
### Phase 1 - Artifact Definition
1. create `ops/pangolin/blueprints/ops-brain/`
...
## Deliverables
1. one canonical monitoring blueprint for `ops-brain`
