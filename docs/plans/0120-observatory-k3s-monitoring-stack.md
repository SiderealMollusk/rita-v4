...
## Goal
Rebuild the 16 GB laptop as the `observatory` and run the full monitoring stack on a bare-metal k3s cluster hosted directly on that laptop.
...
This matches the intended `observatory` role.
...
1. label the laptop as `rita.role=observatory`
2. label future nodes by role:
- `rita.role=platform`
- `rita.role=workload`
3. add node affinity to monitoring workloads so they prefer or require `rita.role=observatory`
...
2. monitoring workloads also use node affinity for `rita.role=observatory`
...
2. create inventory entry for `observatory`
...
1. add `observatory` to inventory/docs
2. create `scripts/2-ops/observatory/` or equivalent domain for laptop runbooks
