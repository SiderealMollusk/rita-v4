...
## Goal
Turn the laptop `observatory` into the first internal monitoring/control-plane node, connect it to the working public `pangolin-server`, and establish the initial access model for dashboards and health surfaces.

## Inputs Confirmed
1. `observatory` is now a working Debian 12 host with SSH, sudo, k3s, Helm, and node label `rita.role=observatory`.
...
## Planning Decisions
### 1. Newt Placement
Install Newt directly on `observatory`.
...
## Scope of This Phase
### In Scope
1. Install Newt on `observatory`.
2. Install Pangolin CLI on the Mac.
3. Verify the `observatory` site registration from `observatory` to `pangolin-server`.
...
3. Create a Pangolin site named `observatory` for `observatory`.
4. Store Newt site credentials in 1Password.
5. Install Newt on the laptop as a persistent systemd service.
6. Confirm `observatory` appears as connected in Pangolin.
...
### Scripts
1. `scripts/2-ops/observatory/01-bootstrap/00-run-all.sh`
2. `scripts/2-ops/observatory/02-services/00-run-all.sh`
3. `scripts/2-ops/observatory/07-verify-cluster.sh`
4. `scripts/2-ops/observatory/10-install-newt.sh`
5. `scripts/2-ops/observatory/11-install-monitoring-stack.sh`
6. `scripts/2-ops/observatory/12-verify-monitoring-stack.sh`
...
## Pass Criteria
1. `observatory` is visible and connected in Pangolin through Newt.
...
1. create/store the `observatory` Pangolin site credentials in 1Password
2. run `scripts/2-ops/observatory/02-services/00-run-all.sh`
