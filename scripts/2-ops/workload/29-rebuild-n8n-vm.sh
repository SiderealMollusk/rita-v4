...
VM_INV="$REPO_ROOT/ops/ansible/inventory/n8n.ini"
...
TARGET_VM_ID="${PROXMOX_N8N_VM_ID:-$(runbook_yaml_get "$HV" workload_pve_n8n_vm_id)}"
TARGET_VM_NAME="${PROXMOX_N8N_VM_NAME:-$(runbook_yaml_get "$HV" workload_pve_n8n_vm_name)}"
TARGET_VM_IP_CIDR="${PROXMOX_N8N_VM_IP_CIDR:-$(runbook_yaml_get "$HV" workload_pve_n8n_ip_cidr)}"
TARGET_VM_GATEWAY="${PROXMOX_N8N_VM_GATEWAY:-$(runbook_yaml_get "$HV" workload_pve_n8n_gateway)}"
TARGET_VM_CORES="${PROXMOX_N8N_VM_CORES:-$(runbook_yaml_get "$HV" workload_pve_n8n_cores)}"
TARGET_VM_MEMORY_MB="${PROXMOX_N8N_VM_MEMORY_MB:-$(runbook_yaml_get "$HV" workload_pve_n8n_memory_mb)}"
TARGET_VM_DISK_GB="${PROXMOX_N8N_VM_DISK_GB:-$(runbook_yaml_get "$HV" workload_pve_n8n_disk_gb)}"
TARGET_VM_BRIDGE="${PROXMOX_N8N_VM_BRIDGE:-$(runbook_yaml_get "$HV" workload_pve_n8n_bridge)}"
TARGET_VM_CIUSER="${PROXMOX_N8N_VM_CIUSER:-$(runbook_yaml_get "$HV" workload_pve_n8n_ciuser)}"
TARGET_VM_SSH_PUBKEY_PATH="${PROXMOX_N8N_VM_SSH_PUBKEY_PATH:-$(runbook_yaml_get "$HV" workload_pve_n8n_ssh_pubkey_path)}"
TARGET_VM_NAMESERVER="${PROXMOX_N8N_VM_NAMESERVER:-$(runbook_yaml_get "$HV" workload_pve_n8n_nameserver)}"
...
[ -n "$TARGET_VM_ID" ] || runbook_fail "Missing workload_pve_n8n_vm_id in $HV"
[ -n "$TARGET_VM_NAME" ] || runbook_fail "Missing workload_pve_n8n_vm_name in $HV"
[ -n "$TARGET_VM_IP_CIDR" ] || runbook_fail "Missing workload_pve_n8n_ip_cidr in $HV"
[ -n "$TARGET_VM_GATEWAY" ] || runbook_fail "Missing workload_pve_n8n_gateway in $HV"
[ -n "$TARGET_VM_SSH_PUBKEY_PATH" ] || runbook_fail "Missing workload_pve_n8n_ssh_pubkey_path in $HV"
...
TARGET_VM_HOST="$(runbook_inventory_get_field "$VM_INV" "n8n-vm" "ansible_host")"
[ -n "$TARGET_VM_HOST" ] || runbook_fail "Could not resolve n8n-vm ansible_host from $VM_INV"

if [ "${PROXMOX_REBUILD_CONFIRM:-}" != "n8n-vm-9303" ]; then
  runbook_fail "Set PROXMOX_REBUILD_CONFIRM=n8n-vm-9303 before rebuilding VM ${TARGET_VM_ID}."
fi
...
echo "[INFO] n8n VM rebuild completed."
