...
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status observatory-newt -n newt >/dev/null"

echo "[INFO] Ensuring monitoring namespace exists"
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl create namespace $MON_NS --dry-run=client -o yaml | kubectl apply -f -"

echo "[INFO] Copying committed monitoring values to observatory"
ansible -i "$INV" observatory -b -m copy -a "src=$PROM_VALUES dest=$REMOTE_PROM_VALUES mode=0644"
ansible -i "$INV" observatory -b -m copy -a "src=$LOKI_VALUES dest=$REMOTE_LOKI_VALUES mode=0644"
ansible -i "$INV" observatory -b -m copy -a "src=$PROMTAIL_VALUES dest=$REMOTE_PROMTAIL_VALUES mode=0644"
ansible -i "$INV" observatory -b -m copy -a "src=$KUMA_VALUES dest=$REMOTE_KUMA_VALUES mode=0644"

echo "[INFO] Adding/updating monitoring Helm repos on observatory"
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm repo add $PROM_REPO_NAME $PROM_REPO_URL >/dev/null 2>&1 || true && helm repo update $PROM_REPO_NAME"
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm repo add $LOKI_REPO_NAME $LOKI_REPO_URL >/dev/null 2>&1 || true && helm repo update $LOKI_REPO_NAME"
if [ "$PROMTAIL_REPO_NAME" = "$LOKI_REPO_NAME" ]; then
  :
else
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm repo add $PROMTAIL_REPO_NAME $PROMTAIL_REPO_URL >/dev/null 2>&1 || true && helm repo update $PROMTAIL_REPO_NAME"
fi
if [ "$KUMA_REPO_NAME" = "$PROM_REPO_NAME" ] || [ "$KUMA_REPO_NAME" = "$LOKI_REPO_NAME" ] || [ "$KUMA_REPO_NAME" = "$PROMTAIL_REPO_NAME" ]; then
  :
else
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm repo add $KUMA_REPO_NAME $KUMA_REPO_URL >/dev/null 2>&1 || true && helm repo update $KUMA_REPO_NAME"
fi
...
for release in "$PROM_RELEASE" "$LOKI_RELEASE" "$PROMTAIL_RELEASE" "$KUMA_RELEASE"; do
  ansible -i "$INV" observatory -b -m shell -a "set -e
$KUBE_ENV
...
if ! ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm upgrade --install $PROM_RELEASE $PROM_CHART -n $MON_NS -f $REMOTE_PROM_VALUES --wait --timeout $MON_TIMEOUT"; then
  echo "[INFO] kube-prometheus-stack release failed or timed out. Dumping diagnostics."
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pods -n $MON_NS -o wide" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pvc -n $MON_NS" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get events -n $MON_NS --sort-by=.lastTimestamp | tail -n 30" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status $PROM_RELEASE -n $MON_NS" || true
  runbook_fail "kube-prometheus-stack install failed or timed out. See diagnostics above."
fi

echo "[INFO] Installing/upgrading Loki"
if ! ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm upgrade --install $LOKI_RELEASE $LOKI_CHART -n $MON_NS -f $REMOTE_LOKI_VALUES --wait --timeout $MON_TIMEOUT"; then
  echo "[INFO] Loki release failed or timed out. Dumping diagnostics."
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pods -n $MON_NS -o wide" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pvc -n $MON_NS" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get events -n $MON_NS --sort-by=.lastTimestamp | tail -n 30" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status $LOKI_RELEASE -n $MON_NS" || true
  runbook_fail "Loki install failed or timed out. See diagnostics above."
fi

echo "[INFO] Installing/upgrading Promtail"
if ! ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm upgrade --install $PROMTAIL_RELEASE $PROMTAIL_CHART -n $MON_NS -f $REMOTE_PROMTAIL_VALUES --wait --timeout $MON_TIMEOUT"; then
  echo "[INFO] Promtail release failed or timed out. Dumping diagnostics."
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pods -n $MON_NS -o wide" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get events -n $MON_NS --sort-by=.lastTimestamp | tail -n 30" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status $PROMTAIL_RELEASE -n $MON_NS" || true
  runbook_fail "Promtail install failed or timed out. See diagnostics above."
fi

echo "[INFO] Installing/upgrading Uptime Kuma"
if ! ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm upgrade --install $KUMA_RELEASE $KUMA_CHART -n $MON_NS -f $REMOTE_KUMA_VALUES --wait --timeout $MON_TIMEOUT"; then
  echo "[INFO] Uptime Kuma release failed or timed out. Dumping diagnostics."
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pods -n $MON_NS -o wide" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pvc -n $MON_NS" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get events -n $MON_NS --sort-by=.lastTimestamp | tail -n 30" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status $KUMA_RELEASE -n $MON_NS" || true
