...
OPS_BRAIN_ROLE_LABEL="$(runbook_yaml_get "$GROUP_VARS" "observatory_k3s_role_label" || true)"
[ -n "$OPS_BRAIN_ROLE_LABEL" ] || runbook_fail "observatory_k3s_role_label missing in $GROUP_VARS"
...
ansible -i "$INV" observatory -m shell -a "test -f \"$OPS_BRAIN_KUBECONFIG\""
...
ansible -i "$INV" observatory -m shell -a "$KUBE_ENV && kubectl get nodes --show-labels | grep -q '$OPS_BRAIN_ROLE_LABEL'"
