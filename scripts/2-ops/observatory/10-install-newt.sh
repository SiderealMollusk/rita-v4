...
echo "[INFO] Copying committed Newt values to observatory"
ansible -i "$INV" observatory -b -m copy -a "src=$VALUES_FILE dest=$REMOTE_VALUES_FILE mode=0644"

echo "[INFO] Ensuring Newt namespace exists"
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl create namespace $NEWT_NAMESPACE --dry-run=client -o yaml | kubectl apply -f -"
...
echo "[INFO] Adding/updating Fossorial Helm repo on observatory"
ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm repo add $NEWT_HELM_REPO_NAME $NEWT_HELM_REPO_URL >/dev/null 2>&1 || true && helm repo update $NEWT_HELM_REPO_NAME"
...
if ! ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm upgrade --install $NEWT_RELEASE $NEWT_HELM_CHART -n $NEWT_NAMESPACE -f $REMOTE_VALUES_FILE --wait --timeout $NEWT_HELM_TIMEOUT"; then
  echo "[INFO] Newt release did not become ready before timeout. Dumping diagnostics."
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get pods -n $NEWT_NAMESPACE -o wide" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get events -n $NEWT_NAMESPACE --sort-by=.lastTimestamp | tail -n 20" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && helm status $NEWT_RELEASE -n $NEWT_NAMESPACE" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl get secret $NEWT_SECRET_NAME -n $NEWT_NAMESPACE -o jsonpath='{.data}'" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl describe deployment ${NEWT_RELEASE}-newt-main -n $NEWT_NAMESPACE" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl logs -n $NEWT_NAMESPACE deployment/${NEWT_RELEASE}-newt-main --tail=200" || true
  ansible -i "$INV" observatory -b -m shell -a "$KUBE_ENV && kubectl describe pod -n $NEWT_NAMESPACE \$(kubectl get pod -n $NEWT_NAMESPACE -l app.kubernetes.io/component=newt -o jsonpath='{.items[0].metadata.name}')" || true
